...
const rootReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case SEARCH_GAMES:
      return {
        ...state,
        searchResults: action.payload.length
          ? action.payload.slice(0, 15)
          : false,
        notFilteredResults: action.payload.length
          ? action.payload.slice(0, 15)
          : false,
        loading: false,
      };
    ...
    case FILTER_BY_GENRE:
      return {
        ...state,
        searchResults: state.searchResults.filter((e) =>
    ...
    case SORT:
      if (/b$/.test(state.sorted))
        return {
          ...state,
          searchResults: [...state.searchResults].sort((b, a) =>
            a[action.payload]
              .toString()
              .localeCompare(b[action.payload].toString())
          ),
          games: [...state.games].sort((b, a) =>
            a[action.payload]
              .toString()
              .localeCompare(b[action.payload.toString()])
          ),
